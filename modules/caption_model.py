        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    # suppress previous word
                    if decoding_constraint and t - divm > 0:
                        logprobsf.scatter_(1, beam_seq_table[divm][t - divm - 1].unsqueeze(1).to(init_logprobs.device), float('-inf'))
                    # suppress UNK tokens in the decoding
                    ...
                    logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it.to(init_logprobs.device), *(
                            args[divm] + [state_table[divm]]))
                    logprobs_table[divm] = F.log_softmax(logprobs_table[divm] / temperature, dim=-1)
